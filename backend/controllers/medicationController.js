...
export const getMedications = async (req, res) => {
    ...
    try {
        ...
        // Fetch medicatons for the specific user or family
        if(familyId != null) {
            const tasks = await Medication.find({ family_id: familyId }); 
            res.status(200).json({ success: true, data: tasks });
        }
        else {
            const tasks = await Medication.find({ user_id: userId }); 
            res.status(200).json({ success: true, data: tasks });
        }
    } catch (error) {
...
// Updates all medications by users ID when they join a family group
